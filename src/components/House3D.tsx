  ...
  return (
    <group position={[plot.x, 0, plot.z]} rotation={[0, plot.rotationY, 0]}>
      ...
      {twoFloors && !flatRoof && (
        <>
          <P
            p={[halfW - 2.2, 0.44 + floorH + 0.2, frontZ + 0.7]}
            s={[3.4, 0.2, 1.5]}
            m={M.frame}
          />
          <P
      ...
      {twoFloors &&
        Array.from({ length: winCount }).map((_, i) => {
          ...
          return (
            <group key={`fw2${i}`}>
              <P p={[x, 0.44 + floorH + 1.6, frontZ]} s={[1.65, 1.45, 0.1]} m={M.frame} g={G.softS} />
              <P
      ...
      {twoFloors && (
        <>
          <P p={[-bw * 0.25, 0.44 + floorH + 1.6, backZ]} s={[1.5, 1.3, 0.1]} m={M.glass} g={G.box} />
          <P p={[bw * 0.25, 0.44 + floorH + 1.6, backZ]} s={[1.5, 1.3, 0.1]} m={M.glass} g={G.box} />
        </>
      )}
      ...
      <P p={[-lotW / 2 + garageW / 2 + 0.4, 0.7, lotD / 2 - 0.6]} s={[garageW, 1.4, 0.16]} m={M.garage} />
      {/* pilaretes do muro */}
      {[-1, 1].map((sg) => (
        <P
          key={`pl${sg}`}
      ...
      <group position={[lotW / 2 - 2, 0, lotD / 2 - 1.4]}>
        <mesh geometry={G.cyl8} material={HOUSE_EXTRA_M.lampPost} position={[0, 0.55, 0]} scale={[0.12, 1.1, 0.12]} />
        <mesh geometry={G.softS} material={HOUSE_EXTRA_M.mailbox} position={[0, 1.2, 0]} scale={[0.4, 0.3, 0.55]} />
      </group>

      {/* luminárias de jardim */}
      {[-1, 1].map((sg) => (
        <group key={`gl${sg}`} position={[doorX + sg * 1.4, 0, frontZ + 2.6]}>
          <mesh geometry={G.cyl8} material={HOUSE_EXTRA_M.lampPost} position={[0, 0.5, 0]} scale={[0.09, 1, 0.09]} />
      ...
      {plot.hasPool && (
        <group position={[bw / 2 - 2.5, 0, bz - bd / 2 - 4.5]}>
          ...
          <mesh geometry={G.cyl8} material={M.frame} position={[-2.9, 0.6, 1.4]} scale={[0.12, 1.2, 0.12]} />
          <mesh geometry={G.cone} material={HOUSE_EXTRA_M.flower} position={[-2.9, 1.35, 1.4]} scale={[1.8, 0.5, 1.8]} />
        </group>
      )}
      ...
      {trees.map((t, i) => (
        <group key={i} position={[t.x, 0, t.z]} scale={t.s}>
          <mesh geometry={G.cyl8} material={M.trunk} position={[0, 0.95, 0]} scale={[0.32, 1.9, 0.32]} />
          <mesh
            geometry={G.blob}
