...
import { P } from "@/components/Props3D";
import { SITE } from "@/lib/loteamento";
...
export function Palm({
  ...
  return (
    <group position={position} scale={scale}>
      <mesh geometry={G.cyl8} material={S.palmTrunk} position={[0, 3.4, 0]} scale={[0.42, 6.8, 0.42]} />
...
export function UtilityPole({
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      <mesh geometry={G.cyl8} material={S.woodPole} position={[0, 4.6, 0]} scale={[0.3, 9.2, 0.3]} />
      ...
      />
      {wire && (
        <primitive object={new THREE.Line(wire, S.wire)} />
      )}
    </group>
  );
}
...
function PowerGrid({ streets }: { streets: { x: number; z: number; width: number; depth: number }[] }) {
  const poles = useMemo(() => {
...
export function Praca({ x, z, width, depth }: { x: number; z: number; width: number; depth: number }) {
  const cx = x + width / 2;
  const cz = z + depth / 2;
  return (
    <group>
      ...
      <group position={[cx - width * 0.18, 0, cz - depth * 0.12]}>
        ...
        <mesh geometry={G.pyramid} material={S.playBlue} position={[0, 2.5, 0]} scale={[3.4, 1.2, 3.4]} />
        {/* escorregador */}
        <mesh
          geometry={G.box}
          material={S.playRed}
      ...
      {[
        ...
        <group key={i} position={[cx + width * fx, 0, cz + depth * fz]}>
          <mesh geometry={G.cyl8} material={S.playMetal} position={[0, 0.5, 0]} scale={[0.16, 1, 0.16]} />
    ...
    <group position={[x, 0, z]}>
      ...
      {[-7, -5.5, -4].map((dx, i) => (
        <group key={i} position={[dx, 0, 4]}>
          <mesh geometry={G.cyl8} material={S.playMetal} position={[0, 2, 0]} scale={[0.08, 4, 0.08]} />
          <P p={[0.5, 2.9, 0]} s={[0.9, 2.2, 0.06]} m={i % 2 ? S.kioskBlue : S.boardStrip} g={G.box} />
        </group>
      ))}
    </group>
  );
